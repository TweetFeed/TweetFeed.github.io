...
- `external_exclusive`: `false` when `external` has at least one entry, `true` when the cross-check resolved and none of the 5 tracked feeds (URLhaus, ThreatFox, MalwareBazaar, USOM, IPsum) lists the value, absent when unresolved or not found - set membership only, not a verdict.
...
- `reg`: for domain/url lookups only - RDAP registrar/creation-date/nameservers plus the IP(s)/ASN resolved at first-seen, plus `age_days_at_report` (UTC days between registration and the first TweetFeed report) and `newly_registered` (true when 30 days or less), covering the 30-day window.
